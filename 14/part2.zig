...
fn oreForFuel(reactions: []Reaction, order: []usize, fuel: u64) u64 {
    for (reactions) |*reaction| {
        reaction.required = 0;
    }

    reactions[findReaction(reactions, "FUEL").?].required = fuel;
...
pub fn main() !void {
    ...
    var i: usize = 0;
    while (true) : (i += 1) {
        const ore = oreForFuel(reactions, order, i);
